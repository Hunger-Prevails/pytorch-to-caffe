...
def to_caffe(output, input_shapes, filename, deletes):
    ...
    def convolution_params(var, layer):
        ...
        layer.convolution_param.num_output = weight_var.variable.size()[0];
    ...
    def parse_convolution(var, index, bottoms, tops):
    ...
    def convert_var():
        for list_index in range(len(node_list)):
            ...
            if is_convolution(var):
                parse_convolution(var, list_index, bottoms, tops);
...
def convert(model, input_shapes, filename):
    ...
    input_variables = [Variable(tensor, requires_grad = True) for tensor in input_tensors];
